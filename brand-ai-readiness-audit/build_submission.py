...
2. Runs all smoke and unit test suites for the marketplace.
3. Aborts packaging if any test suite fails.
4. Packages the CONTENTS of the marketplace root into
   `brand-ai-readiness-audit-submission.zip`.
5. Extracts and verifies the freshly built zip against requirements:
   ...
   - Rejects known stale report files.
6. Outputs a PASS/FAIL summary including zip size and SHA-256 checksum.

Run it from the repository root (where a `brand-ai-readiness-audit/` folder
sits) OR from inside the marketplace root itself (which is the layout a grader
gets after unzipping the submission) — the marketplace directory is detected
automatically.
"""
...
def resolve_marketplace_dir():
    """
    The marketplace root is the directory that directly contains
    `marketplace.json` and `skills/`. It is either the directory this script
    lives in (grader unzips the submission and runs it in place) or a
    `brand-ai-readiness-audit/` sub-directory of it (repository layout).
    """
    here = os.path.dirname(os.path.abspath(__file__))
    if os.path.isfile(os.path.join(here, "marketplace.json")):
        return here
    nested = os.path.join(here, "brand-ai-readiness-audit")
    if os.path.isfile(os.path.join(nested, "marketplace.json")):
        return nested
    return here
...
def package_submission(marketplace_dir, zip_output_path):
    """Zip the contents of the marketplace root (without a wrapper folder)."""
    print("=" * 60)
        ...
        for root, dirs, files in os.walk(marketplace_dir):
            ...
            for file in files:
                ...
                if os.path.abspath(full_path) == this_file:
                    continue  # never bundle the packaged output's own builder twice
...
def main():
    marketplace_dir = resolve_marketplace_dir()
    output_root = os.path.dirname(marketplace_dir) or marketplace_dir
    zip_output_path = os.path.join(output_root, "brand-ai-readiness-audit-submission.zip")

    if not os.path.isfile(os.path.join(marketplace_dir, "marketplace.json")):
        print(f"[ERROR] Marketplace root (with marketplace.json) not found near {marketplace_dir}")
        sys.exit(1)
    ...
    print("=" * 60)
    print("  Status:         PASS (All test suites & verification checks passed)")
